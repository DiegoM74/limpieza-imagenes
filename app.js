// Lógica de la Aplicación - Cover Editor Web
...
let compareViewer, viewerPanZoom, compareHandle, imgResultBefore, imgResultAfter;
let scale = 1;
let panX = 0;
let panY = 0;
let isPanning = false;
...
// Listeners globales para el espacio en el paneo de máscara
...
function setupDropzones() {
    ...
    dropzoneIds.forEach(id => {
        ...
        dz.addEventListener('click', (e) => {
            if (e.target.closest('button') || e.target.closest('.btn-remove-file')) {
                return;
            }
            input.click();
        });
...
function setupMaskEditor() {
    ...
    btnClear.addEventListener('click', () => {
        if (confirm('¿Seguro que deseas borrar toda la máscara pintada?')) {
    ...
    paintCanvas.addEventListener('mousedown', (e) => {
        ...
        if (e.button === 2 || e.button === 1 || currentTool === 'pan' || spacePressed) {
...
function openMaskModal() {
    ...
    if (imgMask) {
        try {
            ...
            try {
                ctxPaint.drawImage(imgMask, 0, 0, paintCanvas.width, paintCanvas.height);
            } catch(err) {}
        }
    }
...
function processImages() {
    ...
    try {
        ...
        log(`Puntos clave - Original: ${kpCover.size()} | Limpia: ${kpClean.size()}`, 'info');
        
        if (kpCover.size() < 4 || kpClean.size() < 4) {
            throw new Error('Pocos puntos de interés detectados. Comprueba la calidad de las imágenes.');
        }
        ...
        if (goodMatches.size() < 4) {
            throw new Error('No se encontraron suficientes coincidencias para alinear las imágenes.');
        }
        ...
        cv.warpPerspective(srcClean, alignedClean, H, dsize, cv.INTER_LINEAR, cv.BORDER_CONSTANT, new cv.Scalar(0,0,0,0));
        ...
        if (colorMatch) {
            ...
            if (dilatePx > 0) {
                ...
                if (nz < 100) {
                    contextMask.delete();
                    contextMask = null;
                    log('Sin contexto suficiente cerca del texto. Usando estadísticas globales.', 'info');
                } else {
                    log(`Usando anillo de contexto de ${nz} píxeles para calcular color.`, 'info');
                }
            }
        ...
        orb.delete(); kpCover.delete(); desCover.delete();
        kpClean.delete(); desClean.delete(); matches.delete();
        goodMatches.delete(); ptsClean.delete(); ptsCover.delete();
        H.delete(); alignedClean.delete(); maskGray.delete();
        colorMatchedClean.delete(); blurredMask.delete();
...
function matchColorStats(src, ref, mask) {
    ...
    // Limpieza
    meanSrc.delete(); stddevSrc.delete();
    meanRef.delete(); stddevRef.delete();
...
function matchColorReinhard(src, ref, mask) {
    ...
    // Limpieza
    srcRGB.delete(); refRGB.delete();
    srcLab.delete(); refLab.delete();
    meanSrc.delete(); stddevSrc.delete();
    meanRef.delete(); stddevRef.delete();
    if (!mask) activeMask.delete();
    correctedLab.delete(); correctedRGB.delete();
    alpha.delete();
...
function matchColorLut(src, ref, mask) {
    ...
    for (let c = 0; c < 3; c++) {
        ...
        let sVec = new cv.MatVector(); sVec.push_back(sCh);
        let rVec = new cv.MatVector(); rVec.push_back(rCh);
        ...
        sHist.delete(); rHist.delete();
        sVec.delete(); rVec.delete();
        lut.delete();
        sCh.delete(); rCh.delete();
        correctedCh.delete();
    }
